...
# Carry index.html's cache-buster versions into the injected CSS/JS URLs so
# browsers refetch after every re-paste instead of serving week-stale cache.
css_v = re.search(r'styles\.css\?(v=\d+)', html)
js_v = re.search(r'main\.js\?(v=\d+)', html)
css_q = f"?{css_v.group(1)}" if css_v else ""
js_q = f"?{js_v.group(1)}" if js_v else ""
...
<link rel="stylesheet" href="{ASSET_BASE}css/styles.css{css_q}">
...
<script src="{ASSET_BASE}js/main.js{js_q}" defer></script>
"""
